...
export const APIRoute = createAPIFileRoute("/api/football")({
  GET: async () => {
    const apiKey = process.env.THE_ODDS_API_KEY;
    if (!apiKey) {
      return Response.json(
        { matches: [], error: "Football odds service is not configured.", fetchedAt: new Date().toISOString() },
        { status: 500 }
      );
    }
    try {
      ...
      return Response.json({ matches, error: null, fetchedAt: new Date().toISOString() });
    } catch (e) {
      console.error("Football fetch failed:", e);
      return Response.json(
        { matches: [], error: "Failed to load football matches. Please try again.", fetchedAt: new Date().toISOString() },
        { status: 500 }
      );
    }
  },
